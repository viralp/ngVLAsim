...
import casatools
from casatools import simulator,msmetadata,ms
...
def cube_generate(inp_img,RA,DEC,delta,alpha,centre_freq,freq,freq_end,total_channels):
 imagename=inp_img[:-5]+'_'+str(RA)+'deg'+'_'+str(DEC)+'deg_cube'+'.fits'
 image=inp_img
 hdu=fits.open(image)[0]
 data=hdu.data
 header=hdu.header
 ...
 DEC=DEC
 alpha = alpha
 nu_0 = float(centre_freq)  # Reference frequency in MHz
 nu_min, nu_max = float(freq),float(freq_end)  # Frequency range in MHz
 n_channels = total_channels  # Define number of frequency channels
 frequencies = np.linspace(nu_min, nu_max, n_channels)  # Generate frequencies
 #cube = np.zeros((n_channels, *data.shape))
 cube = np.zeros((n_channels,xx,yy)) 
 #for i in range(800,1600,100):
 # V2=1e6
 # S2=(S1/(V1**alpha)) * ((V2*i)**alpha)
 # header['CRVAL3']=V2*i
 # fits.writeto(file[:-4]+str(i)+'MHz.FITS',S2,header,overwrite=True)


 for i, nu in enumerate(frequencies):
    #cube[i, :, :] = data * (nu / nu_0) ** alpha
    cube[i, :, :]=(data[:,:]/(nu_0**alpha)) * ((nu)**alpha)
 cdelt1=-(delta/3600.)
 cdelt2=(delta/3600.)      
 ...
 header['CTYPE3'] = 'FREQ    '                                        
 header['CRPIX3'] = 1.                                                  
 header['CRVAL3'] = frequencies[0]  # Starting frequency
 header['CDELT3'] = np.diff(frequencies).mean()  # Frequency step                                             
 header['CUNIT3'] = 'Hz      '                                                            
 header['CTYPE4'] = 'STOKES  '                                                            
 header['CRPIX4'] = 1.                                                  
 header['CRVAL4'] = 1.                                                  
 header['CDELT4'] = 1.                                                  
 header['CUNIT4'] ='        '                                                 
 img=fits.writeto(imagename,cube,header,overwrite=True)
 return img
 ...
 header['CTYPE3'] = 'FREQ    '                                        
 header['CRPIX3'] = 1.                                                  
 header['CRVAL3'] = float(centre_freq)  # Starting frequency
 header['CDELT3'] = float(freq)  # Frequency step                                             
 header['CUNIT3'] = 'Hz      '                                                            
 header['CTYPE4'] = 'STOKES  '                                                            
 header['CRPIX4'] = 1.                                                  
 header['CRVAL4'] = 1.                                                  
...
def channel_phase_corrupt(msname: str = "", deg: tuple = (-10, 10), interval_minutes: int = 5):
    ...
    try:
        # Retrieve columns
        dat = tb.getcol('DATA')   # Shape: (ncor, nchan, nrow)
...
def channel_amp_phase_corrupt(
    ...
    try:
        ...
        for t_start, t_end in zip(time_bins[:-1], time_bins[1:]):
            ...
            dat[:, ch, mask] *= gains  # Broadcasting over pol and row
...
def simulation_noise(vis,t_track,sensitivity):
   msmd.open(vis)
   N_ant=msmd.nantennas()
   nbaseline=N_ant*(N_ant-1)/2.
   total_time=msmd.effexposuretime()['value']/3600 #in hr
   exp=msmd.exposuretime(scan=1)['value']
   nint=total_time*exp
   v=msmd.bandwidths()[0]
   ...
   ms.close()
   scale_factor=sensitivity/np.sqrt(t_track/1.0)
   rms_exp=scale_factor*np.sqrt(npol*nbaseline*nchan*nint) #nint=total on-source time / integration time=3600/5
   return rms_exp/1000.0
...
def cleaning1(vis,imagename,datacolumn,imagesize,cellsize,fits_dir,scale,nterms,parallel):
  clean_output=tclean(vis=vis, imagename=fits_dir+'/'+str(imagename),field='',spw='',
       gridder='standard',wprojplanes=1, pblimit=-0.1, imsize=imagesize, cell=cellsize, specmode='mfs',
       deconvolver='mtmfs', nterms=nterms, scales=scale, datacolumn=datacolumn, smallscalebias=0.9, phasecenter='', restoration=True, restoringbeam='common', 
       interactive=False, niter=1000,  weighting='briggs',robust=0.0,
       stokes='I',threshold='1e-6Jy',sidelobethreshold=2.0,nsigma=4.0,cyclefactor=3.0,parallel=parallel,calcpsf=True, calcres=True,restart=True,savemodel='modelcolumn')
  fits_output=exportfits(imagename=fits_dir+'/'+str(imagename)+'.image.tt0',fitsimage=fits_dir+'/'+str(imagename)+'.image.tt0'+'.fits',overwrite=True)

  return clean_output,fits_output

def cleaning1_cube(vis,imagename,datacolumn,imagesize,cellsize,fits_dir2,scale,nterms,parallel,total_channels,freq,deltafreq):
  
  clean_output=tclean(vis=vis, imagename=fits_dir2+'/'+str(imagename),field='',spw='',
       gridder='standard',wprojplanes=1, pblimit=-0.1, imsize=imagesize, cell=cellsize, specmode='cube',nchan=total_channels,start=freq,width=deltafreq,
       deconvolver='hogbom', nterms=nterms, scales=scale, datacolumn=datacolumn, smallscalebias=0.9, phasecenter='', restoration=True, restoringbeam='common', 
       interactive=False, niter=1000,  weighting='briggs',robust=0.0,
       stokes='I',threshold='1e-6Jy',sidelobethreshold=2.0,nsigma=4.0,cyclefactor=3.0,parallel=parallel,calcpsf=True, calcres=True,restart=True,savemodel='modelcolumn')
  fits_output=exportfits(imagename=fits_dir2+'/'+str(imagename)+'.image',fitsimage=fits_dir2+'/'+str(imagename)+'.image'+'.fits',overwrite=True)
...
def selfcal(vis,caltable,solint): 
 gaincal(vis=vis,
        ...
        parang=False,
        gaintable=[])  
 #plotms(vis=caltable,xaxis='Time',yaxis='phase',iteraxis='antenna',gridrows=2,gridcols=2,showgui=False,
 #        plotfile=caltable+'_time_phase_error.png')
 #plotms(vis=caltable,xaxis='Channel',yaxis='phase',iteraxis='antenna',gridrows=2,gridcols=2,showgui=False,
 #        plotfile=caltable+'_channel_phase_error.png')                 
 applycal(vis=vis,spw='',selectdata=True,
         gaintable=[caltable], field='',interp=['linear'], calwt=False, parang=False,
         applymode='calonly')
         
def selfcal_ap(vis,caltable,solint): 
 gaincal(vis=vis,
        caltable=caltable, spw='',
        solint=solint, combine='',field='',selectdata=True,solnorm=False,
        refant='3',   
        minblperant=4, minsnr=5.0, gaintype='G', calmode='ap', append=False,
        parang=False,
        gaintable=[])  
 #plotms(vis=caltable,xaxis='Time',yaxis='amp',iteraxis='antenna',gridrows=2,gridcols=2,showgui=False,
 #        plotfile=caltable+'_time_amp_error.png')
 #plotms(vis=caltable,xaxis='Channel',yaxis='amp',iteraxis='antenna',gridrows=2,gridcols=2,showgui=False,
 #        plotfile=caltable+'_channel_amp_error.png')                
 applycal(vis=vis,spw='',selectdata=True,
         gaintable=[caltable], field='',interp=['linear'], calwt=False, parang=False,
         applymode='calonly')
         

def simulation(antfile,project_name,model,mapsize,integration,totaltime):
...
def manual_simulation(conf_file,vis,freq,deltafreq,freqresolution,nchannels,integrationtime,total_obs_time,model_img_name,RA,DEC):
  conf_file = conf_file
